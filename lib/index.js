...
module.exports = class Pxls extends EventEmitter {
	...
	setMetadata(width, height, palette, heatmapCooldown, maxStacked, canvasCode) {
		...
		this._width = width;
		this._height = height;
		this._palette = palette.map(e => new PxlsColor(e));
		this._heatmapCooldown = heatmapCooldown;
		this._maxStacked = maxStacked;
		this._canvas = new Uint8Array(width * height);
		this._heatmap = new Uint8Array(width * height);
		this._lookupCache = new Array(width * height);

		if(typeof this._canvasCode !== "undefined" && this._canvasCode !== canvasCode) {
			this.emit("newCanvas");
		}

		this._canvasCode = canvasCode;
	}
